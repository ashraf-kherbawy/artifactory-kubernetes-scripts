...
if [[ $VALUES_YAML == *.yaml ]];then
  ...
  helm upgrade --install artifactory -name jfrog/artifactory -n $NAMESPACE -f $VALUES_YAML ${CHART_VERSION:+--version "$CHART_VERSION"} >/dev/null 2>&1
...
EOF
  ...
  helm upgrade --install artifactory jfrog/artifactory -n $NAMESPACE \
